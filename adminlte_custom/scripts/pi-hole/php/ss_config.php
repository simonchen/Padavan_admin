...
require_once $cur_root.'/auth.php';
require_once $cur_root.'/func.php';
require_once $cur_root.'/database.php';
...
if (!isset($api)) {
    if (isset($_POST['token'])) {
        ...
        log_and_die('Not allowed (login session invalid or expired, please relogin on the Padavan dashboard)!');
    }
}
...
if ($_POST['action'] == 'get_groups') {
    ...
    try {
        $query = $db->query('SELECT * FROM "ss_config";');
        $data = array();
	...
	foreach ($links as $link) {
		...
		if (!is_string($result) || strlen($result) == 0) continue;
		$json = json_decode($result, true);
		//echo json_encode($json);
		if (!$json) continue;
		array_push($rows, array("protocol"=>$json['link_protocol'], "name"=>$json['link_name'], "link"=>$link, "json"=>$result));
	}
...
*/
        foreach ($rows as $row) {
            ...
            if (!$stmt->bindValue(':protocol', $row['protocol'], SQLITE3_TEXT)) {
                throw new Exception('While binding protocol: <strong>'.$db->lastErrorMsg().'</strong><br>Added '.$added.' out of '.$total.' links');
            }
            if (!$stmt->bindValue(':name', $row['name'], SQLITE3_TEXT)) {
                throw new Exception('While binding name: <strong>'.$db->lastErrorMsg().'</strong><br>Added '.$added.' out of '.$total.' links');
            }
            if (!$stmt->bindValue(':link', $row['link'], SQLITE3_TEXT)) {
                throw new Exception('While binding link: <strong>'.$db->lastErrorMsg().'</strong><br>Added '.$added.' out of '.$total.' links');
            }
            if (!$stmt->bindValue(':json', $row['json'], SQLITE3_TEXT)) {
